...
def fuzzy_match(name: str, name_list, cutoff=80) -> str:
    # Returns best match from name_list; if none above cutoff, returns original name
...
def most_recent_saturday(dates: pd.Series) -> date:
    # Try to infer a Saturday present in the upload; else use last Saturday relative to today
    ...
    # fallback: last Saturday relative to today
    today = date.today()
...
# Saturday date selector (defaults smartly)
...
# Map timesheet names to HubSpot closer names (so deals always align to hours spelling)
closer_df['Agent'] = closer_df['Rep'].apply(lambda x: fuzzy_match(x, hubspot_df['CLOSER'].unique()))

# Also map hubspot closer to best hours name, ensuring union alignment
...
# Saturday deals (compare date only)
saturday_deals = (
    hubspot_df.loc[hubspot_df['DealDate'] == sat_date, 'Matched Agent']
    .value_counts().reset_index()
    .rename(columns={'index': 'Agent', 'Matched Agent': 'Saturday Deals'})
)

# First deal of each date (company-wide) → who got it
first_per_date = hubspot_df.sort_values(by='DATE').drop_duplicates(subset=['DealDate'], keep='first')
first_deal_bonus = first_per_date['Matched Agent'].value_counts().reset_index()
first_deal_bonus.columns = ['Agent', 'First Deal Bonus Count']

# Build base list = union of hours & deals so agents with hours but 0 deals appear
...
# Align enroller timesheet names to hubspot enroller names
...
# Concatenate sections (closers first, then enrollers)
